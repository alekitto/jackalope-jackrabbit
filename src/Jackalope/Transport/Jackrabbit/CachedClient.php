...
/**
 * Class to add caching to the client.
 *
...
class CachedClient extends Client
{
     ...
     */
    protected $keySanitizer;

    public function __construct(FactoryInterface $factory, $serverUri, array $caches = array())
    {
        parent::__construct($factory, $serverUri);
    ...
    {
        $cacheKey = "nodes: {$node->getPath()}, ".$this->workspace;
        ...
        if ($node->isNodeType('mix:referenceable')) {
            ...
            $cacheKey = "nodes by uuid: $uuid, ".$this->workspace;
            $this->caches['nodes']->delete($cacheKey);
        }
    }
    ...
    public function getNode($path)
    {
        ...
        $cacheKey = "nodes: $path, ".$this->workspace;
        $cacheKey = $this->sanitizeKey($cacheKey);

        if (false !== ($result = $this->caches['nodes']->fetch($cacheKey))) {
            if ('ItemNotFoundException' === $result) {
                throw new ItemNotFoundException(sprintf('Item "%s" not found in workspace "%s"', $path, $this->workspace));
            }
    ...
    public function getReferences($path, $name = null)
    {
        ...
        $cacheKey = "nodes references: $path, $name, " . $this->workspace;
        $cacheKey = $this->sanitizeKey($cacheKey);
    ...
    public function getWeakReferences($path, $name = null)
    {
        ...
        $cacheKey = "nodes weak references: $path, $name, " . $this->workspace;
        $cacheKey = $this->sanitizeKey($cacheKey);
